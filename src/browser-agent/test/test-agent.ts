...
/**
 * Comprehensive test suite for browser-agent
 */
async function runTests() {
  console.log('🧪 Starting Browser Agent Tests\n');
  
  const results: { test: string; status: 'PASS' | 'FAIL'; time: number; error?: string }[] = [];
  
  // Test 1: Basic Navigation and DOM Indexing
  console.log('Test 1: Basic Navigation and Element Detection');
  const test1Start = Date.now();
  try {
    const agent = new Agent('Navigate to example.com and count all links on the page', {
      headless: true,
      maxSteps: 5
    });
  ...
  // Test 2: Form Interaction (without real credentials)
  console.log('\nTest 2: Form Interaction Capabilities');
  const test2Start = Date.now();
  try {
    const agent = new Agent(
      'Go to example.com and find any input fields, describe what you see', 
      {
        headless: true,
        maxSteps: 5
      }
  ...
  try {
    const agent = new Agent(
      'Take a screenshot of example.com and save the page title to a file called /data/title.txt',
      {
        headless: true,
        maxSteps: 10
      }
  ...
  try {
    const agent = new Agent(
      'Try to click a button that does not exist, then recover and take a screenshot instead',
      {
        headless: true,
        maxSteps: 10
      }
    ...
    // Check if it recovered and took screenshot
