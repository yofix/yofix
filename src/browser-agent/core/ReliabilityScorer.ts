import { StepResult, TaskResult, AgentState } from '../types';
import { TaskPlan, TaskVerification } from './TaskPlanner';
...
export class ReliabilityScorer {
  /**
   * Calculate reliability score for task execution
   */
  ...
  private calculateMetrics(result: TaskResult, state: AgentState): ExecutionMetrics {
    ...
    // Count retries (same action performed multiple times)
    ...
    // Calculate average confidence from thinking
  ...
  private calculateFactors(
    ...
    verifications: TaskVerification[]
  ): ReliabilityScore['factors'] {
    // Task completeness: How many required steps were completed
    ...
    // Error recovery: Penalize for retries but reward if eventually successful
    const errorRecovery = metrics.retryCount === 0 
      ? 1.0 
      : Math.max(0.5, 1 - (metrics.retryCount * 0.1));
    
    // Consistency: How well execution matched the plan
  ...
  private calculateOverallScore(factors: ReliabilityScore['factors']): number {
    // Weighted average of factors
    const weights = {
      taskCompleteness: 0.35,      // Most important
      verificationConfidence: 0.25, // Very important
      actionSuccess: 0.20,
      consistency: 0.15,
      errorRecovery: 0.05
    };
    ...
    for (const [factor, weight] of Object.entries(weights)) {
      score += factors[factor as keyof typeof factors] * weight;
    }
  ...
  private identifyIssues(
    ...
    // Check for unmet success criteria
    ...
    const retryActions = result.steps.filter((step, idx) => 
      result.steps.slice(0, idx).some(s => 
  ...
  private generateRecommendations(
    factors: ReliabilityScore['factors'],
    issues: string[]
  ): string[] {
  ...
  private extractConfidence(thinking: string): number {
    // Look for confidence indicators in thinking
    ...
    const hasHighConfidence = highConfidenceWords.some(word => 
      thinking.toLowerCase().includes(word)
    );
    const hasLowConfidence = lowConfidenceWords.some(word => 
      thinking.toLowerCase().includes(word)
    );
    
    if (hasHighConfidence && !hasLowConfidence) return 0.9;
    if (hasLowConfidence && !hasHighConfidence) return 0.5;
    return 0.7; // Default moderate confidence
  }

  private calculateConsistency(planned: string[], executed: string[]): number {
    if (planned.length === 0) return 0;
    
    // Calculate how many planned actions were executed in order
    let matches = 0;
    let execIndex = 0;
   ...
   */
  generateReport(score: ReliabilityScore): string {
...
${this.getReliabilityAssessment(score.overall)}
  ...
  private getReliabilityRating(score: number): string {
  ...
  private getReliabilityAssessment(score: number): string {
    if (score >= 0.9) {
