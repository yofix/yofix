...
/**
 * Unit tests for browser-agent components (no browser required)
 */
async function runUnitTests() {
  ...
  console.log('\nTest 2: State Manager');
  try {
    const state = new StateManager('Test task');
  ...
  // Test 3: DOM Indexer Script Generation
  console.log('\nTest 3: DOM Indexer');
  try {
    ...
    const found = indexer.findElementsByText(mockDOM as any, 'Click');
  ...
  try {
    ...
    const prompt = promptBuilder.buildTaskPrompt(
      'Click the login button',
      mockState,
      mockDOM,
      [{ name: 'click', description: 'Click an element', parameters: {} }]
    );
  ...
  try {
    ...
    // Test invalid params
    const invalid = registry.validateParams('type', {}); // missing required 'text'
