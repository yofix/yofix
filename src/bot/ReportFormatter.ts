import { ScanResult, VisualIssue, FixResult, CodeFix } from './types';

/**
 * Formats bot responses for GitHub comments
 */
export class ReportFormatter {
   ...
   * Format scan results
   */
  formatScanResult(scan: ScanResult): string {
...
`;
   ...
   */
  private formatNoIssues(scan: ScanResult): string {
...
Your UI changes look good across all viewports.`;
  ...
  /**
   * Format a single issue (short version)
   */
  private formatIssueShort(issue: VisualIssue): string {
    const severityEmoji = {
      critical: '🚨',
      high: '⚠️',
      medium: '💡',
...
`;
   ...
   * Format a single fix
   */
  private formatFix(fix: CodeFix): string {
...
`;
   ...
   */
  formatExplanation(issue: VisualIssue, explanation: string): string {
...
Run \`@yofix fix #${issue.id}\` to generate a fix for this issue.`;
   ...
   */
  generateFullReport(scan: ScanResult): string {
