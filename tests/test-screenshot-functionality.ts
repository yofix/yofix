...
async function testScreenshotFunctionality() {
  ...
  try {
    ...
    if (result.screenshots && result.screenshots.length > 0) {
      console.log(
        "✅ Screenshot capture working - captured",
      ...
      console.log(
        "   Saved test screenshot to test/screenshots/test-screenshot.png",
      );
    } else {
    ...
    if (comparison.diff.hasDifferences) {
      ...
      console.log(
        `   Detected ${comparison.diff.pixelsDiff} pixel differences (${comparison.diff.percentage}%)`,
      );
    ...
    console.log(
      `Screenshot Capture: ${results.screenshotCapture ? "✅" : "❌"}`,
    );
